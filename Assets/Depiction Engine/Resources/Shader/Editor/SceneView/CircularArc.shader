...
Shader "DepictionEngine/Handles Circular Arc"
{
    CGINCLUDE
    ...
    float GetHandleSize(float3 pos)
    {
        ...
        float distance = viewPos1.z;
        
        float t = unity_CameraProjection._m11;
        float fov = atan(1.0 / t ) * 2.0;

        float height = 2.0 * (unity_OrthoParams.w == 1 ? _CameraOrthographicSize : distance * tan(fov * 0.5));

        return height / _CameraPixelHeight;
    }
